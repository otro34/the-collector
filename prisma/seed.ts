 ...
 * Prisma Seed Script
 *
 * This script can clear the database and repopulate it with data from CSV files.
 * It's designed to be run during development to reset the database to a known state.
 *
 * Usage:
 *   npm run db:seed              - Clear DB and reimport all CSV data
 *   npx prisma db seed           - Same as above (Prisma CLI command)
 *
 * Environment variables:
 *   SEED_SKIP_IMPORT=true        - Clear DB only, don't import CSV data
 *   SEED_IMPORT_ONLY=true        - Import CSV data only, don't clear DB
 */
...
const SKIP_IMPORT = process.env.SEED_SKIP_IMPORT === 'true'
const IMPORT_ONLY = process.env.SEED_IMPORT_ONLY === 'true'
 ...
 */
async function checkCSVFiles(): Promise<{
  videogames: boolean
  music: boolean
  books: boolean
}> {
  ...
  const checkFile = async (fileName: string): Promise<boolean> => {
    try {
      await fs.access(path.join(originalDataDir, fileName))
      return true
    } catch {
      return false
    }
  ...
  return {
    videogames: await checkFile('videogames_catalog.csv'),
    music: await checkFile('music_catalog.csv'),
    books: await checkFile('books_manga_comics_catalog.csv'),
  }
}
...
async function importAllCSVData() {
  ...
  const csvFiles = await checkCSVFiles()

  // Import videogames
  if (csvFiles.videogames) {
    ...
    console.log('\n⚠️  Videogames CSV file not found, skipping')
  }
  ...
  if (csvFiles.music) {
    ...
    console.log('\n⚠️  Music CSV file not found, skipping')
  }
  ...
  if (csvFiles.books) {
    ...
    console.log('\n⚠️  Books CSV file not found, skipping')
  }
...
async function main() {
  console.log('\n' + '='.repeat(60))
  console.log('🌱 Prisma Seed Script')
  console.log('='.repeat(60))
  console.log(`Start Time: ${new Date().toLocaleString()}`)

  try {
    // Clear database (unless IMPORT_ONLY is set)
    if (!IMPORT_ONLY) {
      await clearDatabase()
    } else {
      console.log('\n⚠️  Skipping database clear (IMPORT_ONLY=true)')
    }

    // Import CSV data (unless SKIP_IMPORT is set)
    if (!SKIP_IMPORT) {
      await importAllCSVData()
    } else {
      console.log('\n⚠️  Skipping CSV import (SKIP_IMPORT=true)')
    }
